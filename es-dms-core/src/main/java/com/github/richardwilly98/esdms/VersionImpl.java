...
public class VersionImpl extends SecuredItemImpl implements Version {
    ...
    private static final Set<String> readOnlyAttributes = ImmutableSet.of(CREATION_DATE, MODIFIED_DATE);
     ...
     */
    public static class Builder extends SecuredItemImpl.Builder<VersionImpl.Builder> {
	...
	return Objects.toStringHelper(this).add("id", id).add("name", name).add("documentId", documentId).add("versionId", versionId)
	        .add("parentId", parentId).add("current", current).add("description", description).add("attributes", getAttributes())
	        .toString();
    }
}
