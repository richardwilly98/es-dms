...
public class ActivityRestRuntimeTaskServiceTest extends ActivitiRestClientTest {
	...
	public void testAddComment() {
		...
		response = target().path("runtime/tasks").path(id).path("comments").request().post(Entity.entity(comment, MediaType.APPLICATION_JSON_TYPE));
...
//		externalResource.setType("url");
		...
		Response response = target().path("runtime/tasks").path(id).path("attachments").request().post(Entity.entity(externalResource, MediaType.APPLICATION_JSON_TYPE));
		Assert.assertTrue(response.getStatus() == Status.CREATED.getStatusCode());
