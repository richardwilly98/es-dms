...
public class TestRestUserService extends GuiceAndJettyTestBase<UserImpl> {
	...
	private Role getRole(String id) throws Throwable {
		Response response = target().path(RestRoleService.ROLES_PATH).path(id)
	...
	public void testGetUsers() throws Throwable {
		...
		try {
			...
			response = target().path(RestUserService.USERS_PATH)
					.path(RestItemBaseService.SEARCH_PATH)
					.path(UserService.DEFAULT_ADMIN_LOGIN)
					.request(MediaType.APPLICATION_JSON).cookie(adminCookie)
					.get();
	...
	protected UserImpl createUser(String login, String password)
			throws Throwable {
		log.debug(String
