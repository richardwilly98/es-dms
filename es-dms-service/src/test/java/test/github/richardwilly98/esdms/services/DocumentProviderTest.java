...
public class DocumentProviderTest extends ProviderTestBase {
	...
	authenticationService.login(new CredentialImpl.Builder().username(user.getLogin()).password(user.getPassword()).build());
	...
	Assert.assertTrue(attributes.get(DocumentImpl.LOCKED_BY).equals(adminCredential.getUsername()));
