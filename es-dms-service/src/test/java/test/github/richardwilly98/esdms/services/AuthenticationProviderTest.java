...
public class AuthenticationProviderTest extends ProviderTestBase {
	...
	for (User user : users.values()) {
	    Credential credential = new CredentialImpl.Builder().username(user.getLogin()).password(user.getPassword()).build();
	...
	for (User user : users.values()) {
	    ...
	    Assert.assertNotNull(token);
	    for (Role role : roles) {
		boolean hasRole = authenticationService.hasRole(token, role.getId());
		log.debug(String.format("Has %s role: %s", role.getId(), hasRole));
		if (user.getRoles().contains(role)) {
		    Assert.assertTrue(hasRole);
		    for (Permission permission : permissions) {
			boolean hasPermission = authenticationService.hasPermission(token, permission.getId());
			log.debug(String.format("Has %s permission: %s", permission.getId(), hasPermission));
	    ...
	    log.debug(String.format("Has collaborator role: %s", authenticationService.hasRole(token, "collaborator")));
	    log.debug(String.format("Has reader role: %s", authenticationService.hasRole(token, "reader")));
	    log.debug(String.format("Has document:create permission: %s", authenticationService.hasPermission(token, "document:create")));
	}
    }
