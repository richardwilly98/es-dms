...
class SimpleDocument extends DocumentImpl {
	...
	public SimpleDocument build() {
	    this.versions(document.getVersions()).tags(document.getTags()).ratings(document.getRatings()).id(document.getId()).name(document.getName())
		    .disabled(document.isDisabled()).description(document.getDescription()).attributes(document.getAttributes());
