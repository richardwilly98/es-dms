...
public class SimpleVersion extends VersionImpl {
	...
	public SimpleVersion build() {
	    this.versionId(version.getVersionId()).documentId(version.getDocumentId()).file(version.getFile())
		    .parentId(version.getParentId()).current(version.isCurrent()).id(version.getId()).name(version.getName())
		    .disabled(version.isDisabled()).description(version.getDescription()).attributes(version.getAttributes());
